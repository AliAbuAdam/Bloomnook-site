...
//
// Логика приёма платежей ЮKassa. Вынесена в отдельный CommonJS-модуль, потому что
// обработчики routerAdd в PocketBase выполняются в изолированном контексте и НЕ
// видят функции/переменные верхнего уровня файла-хука. Модуль подключается из
// yookassa.pb.js через require(`${__hooks}/yookassa_lib.js`). Секреты берём из
// переменных окружения ($os.getenv), в код не коммитим (см. yookassa.pb.js).
...
// POST /api/yookassa/create-payment — создать платёж для заказа пользователя.
function createPayment(e) {
  ...
  // Сохраняем id платежа и статус ожидания (модуль пишет в обход API-правил).
  try {
...
// POST /api/yookassa/webhook — уведомление ЮKassa о смене статуса платежа.
// Защита: белый список IP + перепроверка статуса через GET (телу не доверяем).
function webhook(e) {
